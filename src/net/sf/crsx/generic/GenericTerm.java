...
//			factory.warning("bad karma", "requesting variable for "+this);
	...
	final public void appendTo(Appendable writer, Map<Variable, String> used, int depth, boolean full, boolean namedProps, boolean variableProps, Set<Variable> omitProps) throws IOException
	{
		...
		appendTermTo(f, used, factory.defined(GenericFactory.NO_LINEAR_VARIABLES), depth,
				false,
	...
	public Unification unify(Pattern that, Set<Variable> contextvars) throws CRSException {
		...
		//Unify
		Unification us = thiscopy.unifyThese(new GenericUnification(thiscopy, thisbound, thatcopy), 
											 thatcopy, 
					 					     rho, 
					 					     new StackedMap<Variable>(), 
					 					     mvars);
     ...
     */
	protected abstract Unification unifyThese 
					(Unification unification, Pattern that,
	 ...
	 */
	protected abstract void generateBitmasks(Set<Variable> legalvars, StackedMap<Variable> rhoprime, StackedMap<Variable> inscope,
			Map<String, Boolean[]> bitmasks, Map<String, GenericMetaApplication> occurrences);
	 ...
	 */
    public abstract GenericTerm makeUnificationCopy(StackedMap<Variable> varrenamings,
	...
	public GenericTerm wrapWithNotProperty(String name, boolean reuse)
	{
		PropertiesConstraintsWrapper pcw =
			(this instanceof PropertiesConstraintsWrapper
	...
	public GenericTerm wrapWithPropertiesOf(Term extra) throws CRSException
	{
		...
		else if (extra instanceof PropertiesConstraintsWrapper)
		{
			...
			return new PropertiesConstraintsWrapper(this, pcw.propertiesRef, pcw.namedPropertyConstraints, pcw.variablePropertyConstraints, pcw.metaPropertyConstraints);
		}
		else if (kind() == Kind.CONSTRUCTION)
		{
			// No meta properties...add properties to constructor.
			return ((GenericTerm) this).reconstruct(Util.wrapWithProperties(factory,  constructor(), Util.propertiesHolder(extra)), true);
		}
		else
		{
