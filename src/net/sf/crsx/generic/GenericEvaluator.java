...
public class GenericEvaluator extends FixedGenericConstruction
{
	 ...
	 */
	public GenericEvaluator(GenericFactory factory, Constructor constructor, Variable[][] binds, GenericTerm[] sub,
			GenericValuation valuation, ExtensibleMap<Variable, Variable> renamings)
	{
		super(factory, constructor, binds, sub);
	...
	private GenericTerm compute(Set<Variable> globals)
	{
		...
		What : switch (what)
		{
				...
				try
				{
					...
					for (int i = 1; i < arity(); ++i)
					{
						...
						if (!Util.isConstant(a))
							return null; // "Sum requires constant arguments?"
			...
			case MINUS :
				...
				try
				{
					if (!Util.isConstant(sub(1)) || !Util.isConstant(sub(2)))
						return null; // "Can only subtract two constant values!"
			...
			case TIMES :
				...
				try
				{
					...
					for (int i = 1; i < arity(); ++i)
					{
						Term a = sub(i);
						if (!Util.isConstant(a))
							return null; // "Product requires constant arguments?"
						Object o = a.constructor().object();
						if (o instanceof Long)
							longProduct *= (Long) o;
						else if (o instanceof Double)
			...
			case DIVIDE :
				...
				computeArguments();
				if (!Util.isConstant(sub(1)) || !Util.isConstant(sub(2)))
					break; // "Can only divide two constant values!"
				try
				{
					double left = Double.parseDouble(Util.symbol(sub(1)));
					double right = Double.parseDouble(Util.symbol(sub(2)));
			...
			case MODULO :
				...
				computeArguments();
				if (!Util.isConstant(sub(1)) || !Util.isConstant(sub(2)))
					break; // "Can only modulo two constant values!"
				try
				{
					Object o1 = sub(1).constructor().object();
					Object o2 = sub(2).constructor().object();
			...
			case DECIMAL :
				...
				computeArguments();
				if (!Util.isConstant(sub(1)))
					break; // "Can only parse constant values!"
				try
				{
			...
			case HEX :
				...
				computeArguments();
				if (!Util.isConstant(sub(1)))
					break; // "Can only hex constant values!"
				try
				{
			...
			case EQ : {
				// $[Equal, t1, t2]
				computeArguments();
				if (Util.isMetaApplication(sub(1)) || Util.isMetaApplication(sub(2)))
					break;
			...
			case LE : {
				...
				computeArguments();
				if (!Util.isConstant(sub(1)) || !Util.isConstant(sub(2)))
					break; // "Can only compare two constant values!"
				double left = Double.parseDouble(Util.symbol(sub(1)));
				double right = Double.parseDouble(Util.symbol(sub(2)));
				return rewrapWithProperties(factory.literal(left <= right));
			}
			...
			case SAME_VARIABLE : {
				// $[SameVariable x y]
				computeArguments();
				if (sub(1).kind() != Kind.VARIABLE_USE || sub(2).kind() != Kind.VARIABLE_USE)
					break; // "Can only compare two variables!"
			...
			case AND : {
				...
				normalizeArguments();
				if (!Util.isConstant(sub(1)) || !Util.isConstant(sub(2)))
					break; // "Can only connect two constant values!"
			...
			case NOT : {
				...
				normalizeArguments();
				if (!Util.isConstant(sub(1)))
					break; // "Can only negate constant values!"
			...
			case REPLACE : {
				...
				computeArguments();
				if (!Util.isConstant(sub(1)) || !Util.isConstant(sub(2)) || !Util.isConstant(sub(3)))
					break;
			...
			case SUBSTRING : {
				computeArguments();
				if (!Util.isConstant(sub(1)) || !Util.isConstant(sub(2)) || (arity() == 4 && !Util.isConstant(sub(3))))
					break;
			...
			case ESCAPE : {
				...
				// $[Escape, string]
				// $[Escape[Q], string]
				String string = sub(0).arity() == 0
						? Util.quoteJava(Util.symbol(sub(1)))
						: quoteWith(Util.symbol(sub(1)), Util.symbol(sub(0).sub(0)));
				return rewrapWithProperties(factory.literal(string));
			}
			...
			case SPLIT : {
				computeArguments();
				if (!Util.isConstant(sub(1)) || !Util.isConstant(sub(2)) || arity() != 3)
					break;
			...
			case TO_FIRST : {
				computeArguments();
				if (!Util.isConstant(sub(1)) || !Util.isConstant(sub(2)) || (arity() > 3 && !Util.isConstant(sub(3))))
					break;
				// $[UptoFirst, string, sep]
				String string = Util.symbol(sub(1));
				String first = Util.symbol(sub(2));
				int index = string.indexOf(first);
				return rewrapWithProperties(
						index < 0 ? (arity() > 3 ? sub(3) : sub(1)) : factory.literal(string.substring(0, index + first.length())));
			}
			...
			case BEFORE_FIRST : {
				...
				int index = string.indexOf(first);
				return rewrapWithProperties(
						index < 0 ? (arity() > 3 ? sub(3) : sub(1)) : factory.literal(string.substring(0, index)));
			}
			...
			case AFTER_FIRST : {
				...
				return rewrapWithProperties(
						index < 0
								? (arity() > 3 ? sub(3) : factory.literal(""))
								: factory.literal(string.substring(index + first.length())));
			}
			...
			case CONSTRUCTION : {
				...
				computeArguments();
				if (arity() == 1 || !Util.isConstant(sub(1)))
					break; // can only generate constant constants [sic]
				return rewrapWithProperties(unquoteConstructionAndArguments(factory, sub(1).constructor().symbol(),
						(sub(0).arity() == 0 ? null : Util.symbol(sub(0).sub(0))), (arity() <= 2 ? factory.nil() : sub(2)),
						renamings));
			}
			...
			case VARIABLE : {
				...
				computeArguments();
				if (arity() == 1 || !Util.isConstant(sub(1)))
					break; // "Can only build free variable with name from constant constructor argument!"
			...
			case META_APPLICATION : {
				// $[M, t, [#list]] ]
				computeArguments();
				if (arity() == 1 || !Util.isConstant(sub(1)) || arity() == 2 || !SequenceIterator.isSequencing(sub(2)))
					break; // "Can only build meta-application with name from first argument constant constructor!"
				return rewrapWithProperties(factory.newMetaApplication(Util.symbol(sub(1)),
						(arity() <= 2 ? NO_TERMS : Util.unconsArray(sub(2), false, renamings))));
			}

			case PROPERTY_NAMED : {
				computeArguments();
				if (!Util.isConstant(sub(1)))
					break; // no go unless name well defined
			...
			case PASS_LOCATION_PROPERTIES : {
				...
				String[] keys =
					{"$FileLocation", "$LineLocation", "$ColumnLocation"};
			...
			case SAVE_TERM :
				...
				if (Util.isConstant(sub(1)))
				{
					...
					try
					{
						...
						sub(2).appendTo(
								w, new HashMap<Variable, String>(), Integer.MAX_VALUE, factory.defined(Factory.SIMPLE_TERMS), true,
								true, null, factory.defined(Factory.SORT_PROPERTIES));
						w.close();
					}
					catch (IOException e)
					{
			...
			case URL :
				...
				if (Util.isConstant(sub(1)) && (arity() == 2 || Util.isConstant(sub(2))))
				{
					...
					try
					{
						...
						return rewrapWithProperties(
								factory.newConstruction(factory.makeConstructor(url), GenericTerm.NO_BINDS, GenericTerm.NO_TERMS));
					}
			...
			case PICK : {
				...
				if (Util.isConstant(sub(1)))
				{
					try
					{
						...
						while (index-- > 0)
						{
							if (!Util.isCons(list.constructor()))
								break What;
							list = list.sub(1);
						}
						if (!Util.isCons(list.constructor()))
			...
			case LIST_LENGTH : {
				...
				if (!Util.isNull(list.constructor()))
					break What;
				return rewrapWithProperties(factory.literal(length));
			}
				...
				if (!Util.propertiesHolder(sub(0)).isMeta())
				{
					...
					else if (Util.isConstant(sub(1)))
					{
						...
						else
						{
							...
							if (lookup != null)
								value = lookup.copy(false, renamings);
						}
					}
					if (value != null)
			...
			case CHECK_SORT_OF : {
				...
				try
				{
					factory.checkSortOf(sort, term, LinkedExtensibleMap.EMPTY_VARIABLE_PROPERTIES, new HashMap<Variable, Term>(),
			...
			case PRINT :
				...
				try
				{
					sub(1).appendTo(System.out, new HashMap<Variable, String>(), Integer.MAX_VALUE,
							factory.defined(Factory.SIMPLE_TERMS), true, true, null, factory.defined(Factory.SORT_PROPERTIES)); // TODO: allow depth?
					System.out.println();
					System.out.flush();
			...
			case TRACE :
				...
				try
				{
					sub(1).appendTo(System.err, new HashMap<Variable, String>(), Integer.MAX_VALUE,
							factory.defined(Factory.SIMPLE_TERMS), true, true, null, factory.defined(Factory.SORT_PROPERTIES));
					System.err.println();
					System.err.flush();
				...
				if (index != -1)
				{
					...
					try
					{
						...
						else
						{
							...
							return factory.newConstruction(
									factory.makeConstructor(CRS.TRYSUCCESS_SYMBOL), GenericTerm.NO_BINDS, subterms);
						}
					}
					catch (Exception e)
					{
						if (what == Primitive.TRYCALL)
						{
							...
							return factory.newConstruction(
									factory.makeConstructor(CRS.TRYFAILURE_SYMBOL), GenericTerm.NO_BINDS, subterms);
						}
						else
						{
	...
	public boolean match(Match match, Term term, ExtensibleSet<Variable> bound, Map<String, Integer> contractionCount, boolean promiscuous, Collection<Variable> once, Collection<Variable> onceSeen)
	{
		...
		switch (what)
		{
			...
			case META_APPLICATION : {
				...
				if (!((Pattern) sub(1)).match(match, factory.literal(term.metaVariable()), bound, contractionCount, promiscuous,
						once, onceSeen))
					return false;
			...
			case PROPERTY_NAMED_NOT : {
				...
				if (what == Primitive.PROPERTY_NAMED)
				{
					...
					if (value == null
							|| !((Pattern) sub(2)).match(match, value, bound, contractionCount, promiscuous, once, onceSeen))
			...
			case PROPERTY_COLLECT : {
				...
				if (!Util.isConstant(sub(1)))
					return false; // "Not constant meta-variable property name?"
				String metavar = Util.symbol(sub(1));
				Term owner = sub(2);
				return ((Pattern) ((GenericTerm) owner).wrapWithPropertiesRef(metavar, false)).match(match, term, bound,
			...
			case MATCH_REGEX : {
				...
				return Util.isConstant(term)
						&& regex.matcher(Util.symbol(term)).matches() && (arity() == 2
								|| ((Pattern) sub(2)).match(match, term, bound, contractionCount, promiscuous, once, onceSeen));
			}
			...
			case IS_INTEGER : {
				// $[IsInteger, #alias]
				return Util.isConstant(term)
						&& INTEGER_REGEX.matcher(Util.symbol(term)).matches() && (arity() == 1
			...
			case VARIABLE_NAME_IS : {
				...
				if (sub(1).kind() != Kind.VARIABLE_USE)
					return false; // fail when there is no variable in the pattern.
			...
			case PLUS : {
				...
				if (!INTEGER_REGEX.matcher(Util.symbol(sub(constantArg))).matches())
					return false;
				final int n = Integer.parseInt(Util.symbol(sub(constantArg)));
				int metaArg = 3 - constantArg;
				if (!Util.isMetaApplication(sub(metaArg)))
					return false;
				// Match the redex subterm against #arg.
				if (!((Pattern) sub(metaArg)).match(match, term, bound, contractionCount, promiscuous, once, onceSeen))
					return false;
				...
				if (!Util.isConstant(argSubstitute.getBody()))
					return false; // submatch must be constant
			...
			case LET :
			case FOR :
				throw new RuntimeException("$[Let not implemented");
			...
			case GET_REF :
				if (Util.isConstant(sub(1)))
				{
					...
					return ((Pattern) (value != null || arity() == 2 ? value : sub(2))).match(match, term, bound, contractionCount,
			...
			case DUMP :
				try
				{
					...
					if (((Pattern) sub(2)).match(match, term, bound, contractionCount, promiscuous, once, onceSeen))
					{
						match.getSubstitute(sub(2).metaVariable()).getBody().appendTo(System.out, used, Integer.MAX_VALUE, full,
			...
			case TRACE :
				...
				{
					sub(1).appendTo(
							System.err, new HashMap<Variable, String>(), Integer.MAX_VALUE, factory.defined(Factory.SIMPLE_TERMS),
							true, true, null, factory.defined(Factory.SORT_PROPERTIES));
					System.err.println();
					System.err.flush();
				}
				catch (IOException e)
			...
			case EQ :
			case NUMEQ :
				if (Util.isMetaApplication(sub(1))
						&& sub(1).arity() == 0
						&& ((Pattern) sub(1)).match(match, term, bound, contractionCount, promiscuous, once, onceSeen))
				{
					...
					else if (Util.isMetaApplication(sub(2))
							&& sub(2).arity() == 0
							&& ((Pattern) sub(2)).match(match, term, bound, contractionCount, promiscuous, once, onceSeen))
					{
	...
	private Term unquoteConstructionAndArguments(GenericFactory factory, String symbol, String sort, Term quotedArguments, ExtensibleMap<Variable, Variable> renamings)
	{
		...
		while (Util.isCons(rest.constructor()))
		{
			...
			while (a.kind() == Kind.CONSTRUCTION
					&& (GenericTerm.BIND.equals(Util.symbol(a)) || GenericTerm.BIND_LINEAR.equals(Util.symbol(a))))
			{
		...
		return factory.newConstruction(
				(SortUtil.isLiteralSort(sort) ? factory.makeLiteral(symbol, sort) : factory.makeConstructor(symbol)),
				binders.toArray(new Variable[0][]), subs.toArray(new GenericTerm[0]));
	}
	...
	private static Term quoteArguments(GenericFactory factory, Term parent)
	{
		...
		for (int i = parent.arity() - 1; i >= 0; --i)
		{
			...
			for (int j = binds.length - 1; j >= 0; --j)
			{
				...
				ts[0] = sub;
				sub = factory.newConstruction(
						factory.makeConstructor(binds[j].promiscuous() ? GenericTerm.BIND : GenericTerm.BIND_LINEAR), bs, ts);
			}
	...
	private static Term quoteProperties(Term term)
	{
		...
		if (ps != null)
		{
			...
			for (Map.Entry<String, Term> e : namedProps.entrySet())
			{
				sink = sink.start(sink.makeConstructor(CRS.CONS_SYMBOL));
				++conses;
				sink = sink.start(sink.makeConstructor("$NP")).start(sink.makeLiteral(e.getKey(), CRS.STRING_SORT)).end().copy(
						e.getValue(), false).end();
			}
			...
			for (Map.Entry<String, Term> e : metaProps.entrySet())
			{
				sink = sink.start(sink.makeConstructor(CRS.CONS_SYMBOL));
				++conses;
				sink = sink.start(sink.makeConstructor("$MP")).startMetaApplication(e.getKey()).endMetaApplication().copy(
						e.getValue(), false).end();
			}
			// List done.
		}
			...
			case CONSTRUCTION : {
				...
				switch (that.kind())
				{
					case CONSTRUCTION : {
						...
						unification.putSubstitute(
								((GenericMetaApplication) this.sub(1)).metaVariable, new Variable[0],
			...
			case FREE_VARIABLES :
				...
				if (sub(0).arity() > 0)
				{
					...
					if (factory.sortAliases.containsKey(argumentSort))
					{
						...
						sub(0).replaceSub(
								0, sub(0).binders(0), factory.newConstruction(
										factory.makeConstructor(realSort), GenericTerm.NO_BINDS, GenericTerm.NO_TERMS));
					}
				}
			default :
		}
			...
			if (options != null && options[i] != null && options[i].size() > 0)
			{
				switch (primitive())
				{
					...
					case FOR :
						...
						{
							sink = sink.start(sink.makeConstructor(CRS.CONS_SYMBOL)); // CONS[
							sink = sink.start(sink.makeConstructor(CRS.REIFY_RULE_OPTION)).start(
									sink.makeLiteral(Builder.DISCARD_OPTION_SYMBOL, CRS.STRING_SORT)).end(); // OPTION["Discard"
							sink = sink.start(sink.makeConstructor(CRS.REIFY_META_APPLICATION)).start(
									sink.makeLiteral(options[i].get(j), CRS.STRING_SORT)).end(); // META-APPLICATION["Name"
							sink = sink.start(sink.makeConstructor(CRS.NIL_SYMBOL)).end().start(
									sink.makeConstructor(CRS.REIFY_NA)).end(); // (), NA
		...
		switch (primitive())
		{
			...
			case IF_ZERO : {
				...
				if (falseUses != null)
				{
					...
					// We could aggregate existing uses but it's not strictly needed
				}
				else
				{
			...
			case LET : {
				// $[Let, #binders, #term, #return]
				// ignore new meta declaration in binders
				sub(3).analyzeMetaUseContractum(uses, subAnalyzers);
				sub(2).analyzeMetaUseContractum(uses, subAnalyzers);
				//sub(0).analyzeMetaUseContractum(uses, subAnalyzers); No need, Let does not take an environment

				List<GenericMetaApplication> binders = new ArrayList<GenericMetaApplication>();
				sub(1).addMeta(binders);

				List<GenericMetaApplication> returnMetas = new ArrayList<GenericMetaApplication>();
				sub(3).addMeta(returnMetas);

				for (GenericMetaApplication binder : binders)
				{
					if (!containsMeta(binder.metaVariable(), returnMetas))
						binder.count = 0;
				}
				break;
			}
			case FOR : {
				// $[For, #binders, #list, #params, #init, #body, #return]
				// First: run standard analysis 
				sub(6).analyzeMetaUseContractum(uses, subAnalyzers);
				sub(5).analyzeMetaUseContractum(uses, subAnalyzers);
				sub(4).analyzeMetaUseContractum(uses, subAnalyzers);
				sub(2).analyzeMetaUseContractum(uses, subAnalyzers);

				List<GenericMetaApplication> binders = new ArrayList<GenericMetaApplication>();
				sub(1).addMeta(binders);
				List<GenericMetaApplication> params = new ArrayList<GenericMetaApplication>();
				sub(3).addMeta(params);

				// Second: consider loop impact on lifetime.
				options = new ArrayList[arity()];

				// Step 1: fix outer metas (#O)
				List<GenericMetaApplication> bodyMetas = new ArrayList<GenericMetaApplication>();
				sub(5).addMeta(bodyMetas);
				List<GenericMetaApplication> returnMetas = new ArrayList<GenericMetaApplication>();
				sub(6).addMeta(returnMetas);

				// Case 1: #O ∈ #return & #O ∈ #body. Mark all #O in body as INF (>1)
				// Case 2: #O ∈ #return & #O ∉ #body. No action.
				// Case 3: #O ∉ #return & #O ∈ #body. Mark all #O in body as INF (>1) and mark all #O in body as DISCARD before #return
				// Case 4: #O ∉ #return & #O ∉ #body. No action

				List<GenericMetaApplication> o = new ArrayList<GenericMetaApplication>(bodyMetas);
				o.removeAll(binders);
				o.removeAll(params);
				for (GenericMetaApplication meta : o)
				{
					final boolean inBody = containsMeta(meta.metaVariable(), bodyMetas);
					final boolean inReturn = containsMeta(meta.metaVariable(), returnMetas);

					if (inBody)
						meta.count = Integer.MAX_VALUE;

					if (!inReturn)
					{
						if (options[6] == null)
							options[6] = new ArrayList<Object>();

						if (options[6].contains(meta.metaVariable()))
							options[6].add(meta.metaVariable());
					}
				}

				// Step 2: fix binders (#B)
				// Case 1:  #B ∈ #return => Error
				// Case 2:  #B ∈ #body. Mark  #B as INF. 
				// Case 3:  #B ∉ #body. Mark #B as DISCARD. 
				for (GenericMetaApplication meta : binders)
				{
					if (containsMeta(meta.metaVariable(), bodyMetas))
						meta.count = Integer.MAX_VALUE;
					else
						assert meta.count == 0;
				}
				...
				// Step 3: fix params (#P)
				// Case 1:  #P ∈ #return & #P ∈ #body. Reanalyze #P in body without #return occurrences.
				// Case 2:  #P ∈ #return & #P ∉ #body. Mark #P as DISCARD in #body.
				// Case 3:  #P ∉ #return & #P ∈ #body. Reanalyze #P in body without #return occurrences. Mark #P as DISCARD in #return.
				// Case 4:  #P ∉ #return & #P ∉ #body. Mark #P as DISCARD in #params.
				for (GenericMetaApplication meta : params)
				{
					final boolean inBody = containsMeta(meta.metaVariable(), bodyMetas);
					final boolean inReturn = containsMeta(meta.metaVariable(), returnMetas);
					...
					if (inBody)
					{
						// (Fixup standard analysis) Compute min occurrence of #P in #body and decrease all so that min use is 1
						int min = Integer.MAX_VALUE;
						for (GenericMetaApplication bodyMeta : bodyMetas)
						{
							if (bodyMeta.metaVariable().equals(meta.metaVariable()))
								min = min > bodyMeta.count ? bodyMeta.count : min;
						}
						min--;
						for (GenericMetaApplication bodyMeta : bodyMetas)
						{
						...
						if (!inReturn) // then discard #P in before #return
						{
							if (options[6] == null)
								options[6] = new ArrayList<Object>();
							if (options[6].contains(meta.metaVariable()))
					...
					else
					{
						...
						{
							if (options[5] == null)
								options[5] = new ArrayList<Object>();
							if (options[5].contains(meta.metaVariable()))
								options[5].add(meta.metaVariable());
						}
						else
							assert meta.count == 0;
					}
	...
	// Whether metas contains metavar
	private boolean containsMeta(String metavar, List<GenericMetaApplication> metas)
	{
		for (GenericMetaApplication meta : metas)
		{
			if (meta.metaVariable().equals(metavar))
				return true;
		}
		return false;
	}
